...
public class ApplyForAppointmentCommandHandler : IRequestHandler<ApplyForAppointmentCommand, Result<Guid>>
{
    ...
    public async Task<Result<Guid>> Handle(ApplyForAppointmentCommand request, CancellationToken cancellationToken)
    {
        ...
        // Notify Teacher
        var teacher = await _uow.GetRepository<Teacher>().Include(t => t.AppUser).FirstOrDefaultAsync(t => t.Id == availability.TeacherId);
        if (teacher != null)
        {
            await _notificationService.SendToUserAsync(teacher.AppUserId.ToString(), "Yeni Randevu Talebi", $"Bir veli {availability.StartTime:dd.MM.yyyy HH:mm} zaman dilimi için randevu başvurusunda bulundu.", "AppointmentRequest");
        }
